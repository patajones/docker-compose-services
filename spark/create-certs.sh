...
# domains
DOMAINS=("docker.local" "minio.docker.local" "s3.minio.docker.local" "spark.docker.local" "worker2.spark.docker.local" "worker1.spark.docker.local")
...
for DOMAIN in "${DOMAINS[@]}"; do
    cat "${OUTPUT}${DOMAIN}.crt" "${OUTPUT}${DOMAIN}.key" $CA_CERT > "${OUTPUT}pem/${DOMAIN}.pem"
    echo "Arquivo PEM gerado para ${DOMAIN}: ${OUTPUT}pem/${DOMAIN}.pem"
done
